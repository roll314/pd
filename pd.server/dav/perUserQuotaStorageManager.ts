...
export class PerUserQuotaStorageManager extends webDav.PerUserStorageManager {
  perUserLimit: {
    [UUID: string]: number;
  };

  constructor(defaultUserQuota: number, private totalDiskSizeBytes: number) {
  ...
  override available(
    ...
    callback(userLimit < 0 ? this.totalDiskSizeBytes : available);
  }
   ...
   * @returns void
   */
  setUserLimit(user: webDav.IUser, limit: number): void {
    if (user === undefined) {
      return;
    }
   ...
   * @returns void
   */
  setUserReserved(user: webDav.IUser, storage: number): void {
    if (user === undefined) {
      return;
    }
  ...
  getUserReserved(user: webDav.IUser): number {
    if (user === undefined) {
      return -1;
    }
